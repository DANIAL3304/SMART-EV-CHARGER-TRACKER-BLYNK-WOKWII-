...
void chip_timer_event(void *user_data) {
  ...
  // Send exactly 3 lines every second to ensure TinyGPS++ gets a full update
  for (int i = 0; i < 3; i++) {
    // Double check to make sure don't go out of bounds of the 75-sentence array
  ...
  // Reset index if  have finished all 25 coordinate points (75 sentences total)
  if (chip->gps_tx_index >= LEN(gps_tx_data)) {       
    chip->gps_tx_index = 0;
  }
}